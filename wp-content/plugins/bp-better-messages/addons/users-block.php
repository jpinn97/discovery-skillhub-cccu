    ...
    class Better_Messages_Block_Users
    {
        ...
        public function disable_start_thread_for_blocked_users(&$args, &$errors){
            ...
            foreach($recipients as $recipient) {
                ...
                $is_blocked_1 = $this->is_user_blocked(Better_Messages()->functions->get_current_user_id(), $user->ID);
                if ($is_blocked_1){
                    $errors[] = sprintf(_x('%s blocked by you', 'Error when starting new thread but user blocked', 'bp-better-messages'), Better_Messages()->functions->get_name($user->ID));
                    continue;
                }

                $is_blocked_2 = $this->is_user_blocked($user->ID, Better_Messages()->functions->get_current_user_id());
                if ($is_blocked_2){
                    $errors[] = sprintf(_x('%s blocked you', 'Error when starting new thread but user blocked', 'bp-better-messages'), Better_Messages()->functions->get_name($user->ID));
                    continue;
                }
            }
        }
