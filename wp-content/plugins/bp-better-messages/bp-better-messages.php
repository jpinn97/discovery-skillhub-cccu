...
if ( !class_exists( 'Better_Messages' ) && !function_exists( 'bpbm_fs' ) ) {
    class Better_Messages
    {
        ...
        public function enqueue_admin_css()
        {
            ...
            if ( $current_screen->id === 'better-messages_page_better-messages-viewer' ) {
                wp_enqueue_style(
                    'better-messages-selectize',
                    plugins_url( 'assets/admin/selectize.min.css', __FILE__ ),
                    [],
                    '0.15.2'
                );
            }
        ...
        public function enqueue_admin_js()
        {
            $current_screen = get_current_screen();
            $include = false;
            if ( $current_screen->id === 'better-messages_page_better-messages-mobile-app' ) {
                $include = true;
            }
            ...
            if ( $current_screen->id === 'better-messages_page_better-messages-viewer' ) {
                wp_enqueue_script(
                    'better-messages-selectize',
                    plugins_url( 'assets/admin/selectize.min.js', __FILE__ ),
                    [ 'jquery' ],
                    $this->version
                );
                $include = true;
