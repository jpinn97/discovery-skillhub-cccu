        ...
        public function notifications_sender()
        {
            ...
            if( count( $user_without_last_activity ) > 0 ){
                foreach( $user_without_last_activity as $user_id ){
                    $last_activity = Better_Messages()->functions->get_user_meta( $user_id, 'last_activity', true );

                    if( ! empty( $last_activity ) ){
                        Better_Messages()->functions->update_user_meta( $user_id, 'bpbm_last_activity', $last_activity );
                    } else {
                        Better_Messages()->functions->update_user_meta( $user_id, 'bpbm_last_activity', gmdate( 'Y-m-d H:i:s',  0 ) );
                    }
                }
            }
            ...
            foreach ( array_unique( wp_list_pluck( $unread_threads, 'user_id' ) ) as $user_id ) {
                $meta = Better_Messages()->functions->get_user_meta( $user_id, 'bp-better-messages-last-notified', true );
            ...
            foreach ( $unread_threads as $thread ) {
                ...
                if ( ! isset( $last_notified[ $user_id ][ $thread_id ] ) || ( $thread->last_id > $last_notified[ $user_id ][ $thread_id ] ) ) {
                    ...
                    $last_notified[ $user_id ][ $thread_id ] = $thread->last_id;
                    Better_Messages()->functions->update_user_meta( $user_id, 'bp-better-messages-last-notified', $last_notified[ $user_id ] );
                }
